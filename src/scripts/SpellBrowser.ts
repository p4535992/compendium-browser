// // import { SETTINGS, MODULE_NAME } from './module/settings';

// /**
//  * @author Felix Müller aka syl3r86
//  * @version 0.2.0
//  */
// export class SpellBrowser extends Application {
...
//             this.classList = await fetch('modules/'+this.MODULE_NAME+'/packs/spell-classes.json').then(result => {
//                 return result.json();
//             }).then(obj => {
...
//             this.packList = await fetch('modules/'+this.MODULE_NAME+'/packs/item-packs.json').then(result => {
//                 return result.json();
//             }).then(obj => {
...
//             this.subClasses = await fetch('modules/'+this.MODULE_NAME+'/packs/sub-classes.json').then(result => {
//                 return result.json();
//             }).then(obj => {
...
//             let path:string = $(ev.target).parents('.filter').data('path');
//             let key:string = path.replace(/\./g, '');
//             let filterType:string = 'multiSelect';
//             let itemType:string = $(ev.target).parents('.tab').data('tab');
//             let valIsArray:any = $(ev.target).parents('.filter').data('valisarray');
//             if (valIsArray === 'true') valIsArray = true;
//             let value:string = $(ev.target).data('value');

//             let filterTarget:string = `${itemType}Filters`;
//             let filter:string = this[filterTarget].activeFilters[key];
...
//         game.settings.register(this.MODULE_NAME, "settings", {
...
//         let settings = game.settings.get(this.MODULE_NAME, 'settings');
...
//         if (game.user.isGM) {
//             game.settings.set(this.MODULE_NAME, 'settings', defaultSettings);
...
//     saveSettings() {
//         game.settings.set(this.MODULE_NAME, 'settings', this.settings);
//     }
