...
export class SpellBrowser extends Application {
    ...
    async loadItems() {
        ...
        if (this.classList === undefined) {
            this.classList = await fetch(`modules/${MODULE_NAME}/packs/spell-classes.json`).then(result => {
                return result.json();
            }).then(obj => {
        ...
        if (this.packList === undefined) {
            this.packList = await fetch(`modules/${MODULE_NAME}/packs/item-packs.json`).then(result => {
                return result.json();
            }).then(obj => {
        ...
        if (this.subClasses === undefined) {
            this.subClasses = await fetch(`modules/${MODULE_NAME}/packs/sub-classes.json`).then(result => {
                return result.json();
            }).then(obj => {
    ...
    saveSettings() {
        game.settings.set(MODULE_NAME, 'settings', this.settings);
    }
